...
export default function ChartPage() {
  ...
  useEffect(() => {
    void LocalDb.getCurrentGroup().then(async (g) => {
      ...
      setMedMaster(await LocalDb.getMedications(g.group_id));
    });
  }, [nav]);

  useEffect(() => {
    if (!selUser) return;
    void Promise.all([LocalDb.listRecords(selUser), LocalDb.listEvents(selUser)]).then(
      ([r, m]) => {
        setRecords(r);
        setMeds(m.filter((e) => e.event_type === "medication"));
      }
    );
  }, [selUser]);
  ...
  // 投薬イベントから表示名を解決
  const getMedName = (event: EventRow) => {
    const medId = extractMedId(event);
    if (medId) {
      const found = medMaster.find((m) => m.uuid === medId);
      if (found) return found.name;
    }
    // 旧フォーマットの payload に名前が直接入っているケース
  ...
  const chartData = useMemo(() => {
    ...
    // medMaster を依存に含めることで、マスタ取得後に名前が再解決される
  ...
  return (
    <div
      style={{ minHeight: "100dvh", background: "#f4f5f7", display: "flex", flexDirection: "column" }}
    >
      <header
        style={{
          height: 56,
      ...
      >
        <button
          onClick={() => nav("/")}
          style={{ border: "none", background: "transparent", color: "white", fontSize: 20, width: 40 }}
        >
          ←
        </button>
      ...
      <div style={{ flex: 1, padding: 16, overflowY: "auto" }}>
        ...
        {combinedList.length === 0 && (
          <div style={{ textAlign: "center", color: "#ccc", padding: 20 }}>記録がありません</div>
        )}
