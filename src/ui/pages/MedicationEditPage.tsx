...
export default function MedicationEditPage() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      loadedData.schedule = schedule as Medication["schedule"];
  ...
  const handleAskAi = async () => {
    ...
    try {
      ...
      const safeInteraction = parseInteraction(result.interaction) ?? undefined;
      setFormData((prev) => ({
        ...prev,
  ...
  // スケジュール設定用ヘルパー
  const updateSchedule = (key: string, val: unknown) => {
    setFormData((prev) => ({
      ...prev,
      schedule: { ...prev.schedule, [key]: val } as Medication["schedule"],
    }));
  };
        ...
        {isOpenDetails && (
          <>
                  ...
                  >
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <input
                        type="number"
                        value={
                          formData.schedule?.reminder_minutes
                            ? formData.schedule.reminder_minutes / 60
                            : formData.schedule?.interval_hours || 0
                        }
                        onChange={(e) =>
                          updateSchedule(
                            "reminder_minutes",
            ...
            <div style={{ ...styles.card, border: "2px solid #e0f2fe" }}>
              ...
              {formData.ai_interaction &&
                formData.ai_interaction.status &&
                formData.ai_interaction.status !== "none" && (
                  <div
                    style={{
                      background:
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    ai_tags: e.target.value.split(",").map((t) => t.trim()),
                  })
                }
                style={styles.input}
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        setFormData({
                          ...formData,
                          taste_rating: val as Medication["taste_rating"],
                        })
                      }
                      style={{
                        flex: 1,
              ...
              >
                ...
                <div
                  onClick={() =>
                    setFormData({
                      ...formData,
                      show_in_input: formData.show_in_input === 1 ? 0 : 1,
                    })
                  }
                  style={{
                    width: 50,
